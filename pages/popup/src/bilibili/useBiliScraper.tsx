...
const convertBiliDataToCSV = (data: BiliVideoData[]) => {
...
export const isBilibiliPage = (url: string) => url.includes('https://search.bilibili.com');

const scrapeBiliVideoData = () => {
  const results: BiliVideoData[] = [];
  ...
  const handleScrapeBiliData = useCallback(async () => {
    ...
    try {
      ...
      if (results && results[0]?.result) {
        const data = results[0].result as BiliVideoData[];
